...
class FavoriteWatcher(commands.Cog, name="FavoriteWatcherCog"):
    ...
    async def check_new_favorites(self):
        ...
        for channel_id, exclude_pattern in [
            (self.fav_ch, self.fav_ch_exclude),
            (self.sfw_fav_ch, self.sfw_fav_ch_exclude),
            (self.vore_fav_ch, self.vore_fav_ch_exclude),
        ]:
            logging.debug(f"Checking favorites for channel {channel_id} with exclude pattern {exclude_pattern}")
